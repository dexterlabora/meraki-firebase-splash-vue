...
var apiKey = process.env.API_KEY || ""; //|| configs.apiKey;
...
const summary = async () => {
  ...
  try {
    ...
    let orgs = await meraki
      .get("/organizations", { transformResponse: [data => data] })
      .then(res => {
        return res.data;
      });
    ...
    for (let o of orgs) {
      try {
        let nets = ([] = await meraki
          .get("/organizations/" + o.id + "/networks")
          .then(res => {
            return res.data;
          }));
        ...
        for (let n of nets) {
          try {
            let policies = ([] = await meraki
              .get("/networks/" + n.id + "/groupPolicies")
              .then(res => {
                return res.data;
              }));
